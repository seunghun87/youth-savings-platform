...
const scenarios = [
  ...
  ["적금 1개 사용자", {target:50000000,current:5000000,unallocated:200000,accounts:[account()]}, {budget:500000,paid:300000,futurePrincipal:3600000}],
  ...
  ["만기 완료 포함 사용자", {target:40000000,current:15000000,unallocated:250000,accounts:[account({status:"만기완료",monthly:500000,paid:500000,remainingMonths:6}),account({monthly:200000})]}, {budget:450000,paid:300000,futurePrincipal:2400000}],
  ...
  ["큰 목표 사용자", {target:1000000000,current:100000000,unallocated:3000000,accounts:[account({monthly:2000000,remainingMonths:60})]}, {budget:5000000,baseMonths:180,futurePrincipal:121700000}],
...
test("이번 달 미납이면 이번 달 약정액과 남은 회차 원금을 모두 포함한다",()=>assert.equal(remainingPaymentPrincipal(300000,0,5),1800000));
test("이번 달 부분납입이면 미납 차액만 추가한다",()=>assert.equal(remainingPaymentPrincipal(300000,120000,5),1680000));
test("이번 달 완납·초과납입은 음수 잔액을 만들지 않는다",()=>{assert.equal(remainingPaymentPrincipal(300000,300000,5),1500000);assert.equal(remainingPaymentPrincipal(300000,500000,5),1500000)});
test("내 적금 목표 기여 예상은 현재 원금과 미래 원금·이자·지원금을 모두 포함한다",()=>assert.equal(accountProjectedValue({balance:1000000,monthly:300000,paid:0,remainingMonths:2,interest:50000,support:100000}),2050000));
