...
export default function SavingsPrototype({user,onSignOut,initialTab="home"}:{user:User;onSignOut:()=>void;initialTab?:Tab}) {
  ...
  useEffect(()=>{if(selectedProduct)setEnrollmentMonths(String(selectedProduct.recommendation?.calculation_period_months??selectedProduct.minPeriod))},[selectedProduct?.id]);
  useEffect(()=>{
    if(!userState)return;
  ...
  const enrollProduct=async(p:ProductView)=>{const amount=Number(enrollmentAmount),months=Number(enrollmentMonths);if(!Number.isInteger(amount)||amount<p.minMonthly||(p.maxMonthly!==null&&amount>p.maxMonthly)){setNotice(`납입액은 ${p.minMonthly.toLocaleString()}원${p.maxMonthly?`~${p.maxMonthly.toLocaleString()}원`:" 이상"}으로 입력해주세요.`);return;}if(!Number.isInteger(months)||months<p.minPeriod||months>p.maxPeriod){setNotice(`가입 기간은 ${p.minPeriod}~${p.maxPeriod}개월로 입력해주세요.`);return;}const startedAt=seoulDateKey();try{await addEnrolledProduct(clientId,{product_id:p.id,product_name:p.name,bank:p.org,status:"가입완료",started_at:startedAt,matures_at:addMonthsToDateKey(startedAt,months),interest_rate:p.rate,monthly_amount:amount,contribution_type:p.contributionType,payment_frequency:p.paymentFrequency,min_amount:p.minMonthly,max_amount:p.maxMonthly??undefined,installment_step_amount:p.installmentStep??undefined});await reloadState();setSelectedProduct(null);setNotice("가입 기간과 약정액을 내 플랜에 반영했어요.");}catch(e){setNotice(e instanceof Error?e.message:"상품을 추가하지 못했습니다.");}};
