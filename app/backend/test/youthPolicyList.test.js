...
const ROWS = [
  // 나이 범위가 넓은 충족 정책
  { id: '1', plcy_no:'2026-1', name: '전국 청년 지원', operating_org:'정부', zip_cd:null, min_age: 19, max_age: 39, income_max: null, income_min: null,
    eligibility_text: null, exclusion_text: null, income_etc: null, keywords: null, category_large: '복지문화' },
  { id: '5', plcy_no:'2025-1', name: '전국 청년 지원', operating_org:'정부', zip_cd:null, min_age: 19, max_age: 39, income_max: null, income_min: null,
    eligibility_text: null, exclusion_text: null, income_etc: null, keywords: null, category_large: '복지문화' },
  // 나이 범위가 좁은 충족 정책 → 더 특정 대상이므로 위로
  { id: '2', plcy_no:'2', name: '만 25~27세 특별지원', zip_cd:'11000', min_age: 25, max_age: 27, income_max: null, income_min: null,
    eligibility_text: null, exclusion_text: null, income_etc: null, keywords: null, category_large: '주거' },
  // 소득 상한 초과 → 미충족
  { id: '3', plcy_no:'3', name: '저소득 청년 지원', zip_cd:'41000', min_age: 19, max_age: 39, income_max: 2000, income_min: null,
    eligibility_text: null, exclusion_text: null, income_etc: null, keywords: '주거', category_large: '주거' },
  // 참여제한 문구가 있어 자동 판정 불가 → 확인 필요
  { id: '4', plcy_no:'4', name: '청년 창업 지원', zip_cd:null, min_age: 19, max_age: 39, income_max: null, income_min: null,
...
test('충족 > 확인 필요 > 미충족 순으로, 같은 상태면 좁은 연령대가 먼저 온다', async () => {
  const result = await listYouthPolicies({ age: 26, personalIncome: 3000 });
  assert.deepEqual(result.items.map(p => p.id), ['2', '1', '4', '3']);
});
...
test('limit으로 응답 건수를 제한한다', async () => {
  ...
  // 정렬 후 상위 2건이어야 한다
  assert.deepEqual(result.items.map(p => p.id), ['2', '1']);
});

test('나이·소득이 없으면 판정하지 않고 정렬도 바꾸지 않는다', async () => {
  const result = await listYouthPolicies({});
  assert.deepEqual(result.items.map(p => p.id), ['1', '2', '3', '4']);
  assert.equal(result.items.every(p => p.status === null && p.reason === null), true);
});

test('검색어는 정책명과 키워드 양쪽에서 찾는다', async () => {
  const byName = await listYouthPolicies({ age: 26, keyword: '창업' });
  assert.deepEqual(byName.items.map(p => p.id), ['4']);

  // id 3은 이름에 '주거'가 없지만 keywords에 있다
  const byKeyword = await listYouthPolicies({ age: 26, keyword: '주거' });
  assert.equal(byKeyword.items.some(p => p.id === '3'), true);
});

test('거주 시도 정책과 전국 정책만 남기고 재공고 중복을 제거한다', async () => {
  const result = await listYouthPolicies({ age:26, personalIncome:3000, city:'서울특별시 강남구' });
  assert.deepEqual(result.items.map(p=>p.id), ['2','1','4']);
  assert.equal(result.items.some(p=>p.id==='5'), false);
  assert.equal(result.items.some(p=>p.id==='3'), false);
});
